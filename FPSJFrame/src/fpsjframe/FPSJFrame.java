...
// Attempted to insert image and use for floor
...
public class FPSJFrame extends JPanel implements KeyListener, Runnable {
    ...
    private Image floor; // images used for game 
    private Image wall;
    private Image roof;
    ...
    public FPSJFrame() {
            ...
            try {
            floor = ImageIO.read(new File("/Users/daniel/Downloads/A Lions Tale/School/Game resources/floor.png"));
                   } catch (IOException e) {
                     e.printStackTrace();
                    }
          
      try {
            wall = ImageIO.read(new File("/Users/daniel/Downloads/A Lions Tale/School/Game resources/wall.png"));
                   } catch (IOException e) {
                     e.printStackTrace();
                    }
    ...
    // Draw ceiling
        g.setColor(Color.BLACK);
        for (int y = 0; y < nCeiling; y++) {
            g.drawLine(x, y, x, y);
        }

        // Draw wall with texture
        for (int y = nCeiling; y < nFloor; y++) {
            float sampleY = ((float) (y - nCeiling) / (nFloor - nCeiling)) * wall.getHeight(null);
            g.drawImage(wall, x, y, x + 1, y + 1, (int) (x % wall.getWidth(null)), (int) sampleY,
                        (int) (x % wall.getWidth(null)) + 1, (int) sampleY + 1, null);
        }

        // Draw floor with texture
        for (int y = nFloor; y < nScreenHeight; y++) {
            g.drawImage(floor, x, y, x + 1, y + 1, x % floor.getWidth(null), y % floor.getHeight(null),
                        (x % floor.getWidth(null)) + 1, (y % floor.getHeight(null)) + 1, null);
        }
